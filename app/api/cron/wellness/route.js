...
import { buildWellnessPrompt } from '@/lib/prompts';
...
import { WELLNESS_TOPICS } from '@/lib/cronTopics';
...
export async function GET(request) {
  ...
  const base = WELLNESS_TOPICS[Math.floor(Math.random() * WELLNESS_TOPICS.length)];
  const topic = `${base} (Guida ${new Date().getFullYear()})`;

  const raw = await callGemini(buildWellnessPrompt(topic));
  if (!raw) {
    return NextResponse.json({ error: 'Risposta AI vuota.' }, { status: 502 });
  }

  const content = cleanOutput(raw);
  const { cover, inline } = buildArticleImages(topic, 'Benessere');
  ...
  await sendNewArticleEmail({ title: topic, slug, category: 'Benessere' });
