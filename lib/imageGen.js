...
// any failure (including taking too long - cron routes run on a tight
// serverless time budget, so this must never be the reason an article
// fails to generate) so the caller can fall back to templatePrompts().
