...
export function getCategoryFallback(category) {
  if (category === 'Farmaco') return DRUG_FALLBACK;
  if (category === 'Problemi Frequenti') return PROBLEMS_FALLBACK;
  return WELLNESS_FALLBACK;
}
...
export function buildArticleImages(title, category) {
  ...
  if (category === 'Farmaco') {
    cover = pickFromMap(DRUG_MAP, title) || pollinationsUrl(`${title} medicine box packaging white background studio product photography`, seed);
    inlinePrompt = `${title} medicine pack close up detail`;
  } else if (category === 'Problemi Frequenti') {
    cover = pickFromMap(PROBLEMS_MAP, title) || pollinationsUrl(`${title} medical clinic doctor consultation`, seed);
    inlinePrompt = `${title} medical consultation healthcare closeup`;
  } else {
    cover = pickFromMap(WELLNESS_MAP, title) || pollinationsUrl(`${title} wellness healthy lifestyle nature`, seed);
